				...
				for file in sorted_nicely(files):
						...
						try:
								...
								if cont > 9:
									...
									desenhar_circulo(anterior, atual, cur_label, cont)
...
def desenhar_circulo(anterior, atual, cur_label, numero):
	...
	# Capture from webcam
		...
		if contours:
				...
				#ROI = ROI[y:y+area_of_circle,x:x+area_of_circle]
				...
				#cv2.imwrite('dataset/resultado/'+cur_label+str(numero)+'.jpg', ROI)
				return ROI
		else:
