...
def desenhar_roi1(atual, anterior, nome_classe):
	...
	if contours:
		...
		huMoments= -np.sign(huMoments)*np.log10(np.abs(huMoments))#precisa?
		...
		ra= int(maxVal)
		centerMax= maxLoc
		areaMaxInscri = (ra*2)*3.14
		areaMaxInscri= "%.2f" % areaMaxInscri
		...
		rb = int(radius)
		areaMinEnclo = (rb*2)*3.14
		areaMinEnclo= "%.2f" % areaMinEnclo
...
def train():
		...
		for name in nameClasse:
			...
			for file in sorted_nicely(files):
				...
				limite= 300 #limite de imagem a ser processada
				im.append(atual)
				try:
